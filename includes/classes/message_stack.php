...
class messageStack extends base {
  ...
  function add($class, $message, $type = 'error') {
    ...
    if (strlen($message) > 0) {
      if ($type == 'error') {
        $theAlert = array('params' => 'class="mj-boxwarning"', 'class' => $class, 'text' => $message);
      } elseif ($type == 'warning') {
        $theAlert = array('params' => 'class="mj-boxwarning"', 'class' => $class, 'text' => $message);
      } elseif ($type == 'success') {
        $theAlert = array('params' => 'class="mj-boxdownload"', 'class' => $class, 'text' => $message);
      } elseif ($type == 'caution') {
        $theAlert = array('params' => 'class="mj-boxhint"', 'class' => $class, 'text' => $message);
      } else {
        $theAlert = array('params' => 'class="mj-boxinfo"', 'class' => $class, 'text' => $message);
      }
