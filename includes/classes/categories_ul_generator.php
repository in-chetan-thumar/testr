    ...
    function buildBranch($parent_id, $level, $submenu=true, $parent_link='')
    {
        ...
        if (($this->data[$parent_id])) {
            foreach($this->data[$parent_id] as $category_id => $category) {
				...
				if($category_id=='81')
				{
				   $result .= str_repeat($this->spacer_string, $this->spacer_multiplier * 1) . '<a href="' . zen_href_link('shopping_cart&pg=store') . '">';
				}
				else if($category_id=='82')
				{
				  $result .= str_repeat($this->spacer_string, $this->spacer_multiplier * 1) . '<a href="' . zen_href_link('account_history&pg=store') . '">';
				}
				else
				{
                  $result .= str_repeat($this->spacer_string, $this->spacer_multiplier * 1) . '<a href="' . zen_href_link(FILENAME_DEFAULT, 'cPath=' . $category_link.'&pg=store') . '">';
				 }
