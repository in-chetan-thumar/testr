...
if ((PROJECT_VERSION_MAJOR == 1 && substr(PROJECT_VERSION_MINOR, 0, 3) <= 3.8)) {
  require(DIR_WS_CLASSES . 'order_138.php');
} else {
  ...
  class order extends base {
    ...
    function query($order_id) {
      ...
      // added shipping_telephone below
      $order_query = "select customers_id, customers_name, customers_company,
                           ...
                           date_purchased, orders_status, last_modified, order_total, order_tax, ip_address, shipping_telephone
                          from " . TABLE_ORDERS . "
      ...
      if (FEC_GIFT_WRAPPING_SWITCH == 'true') {
        // Gift wrap
        $wrap_query = "select orders_products_id, wrap, wrap_config, wrap_name from " . 
        ...
        while (!$wraplist->EOF) {
          $this->wrapped [] = array('orders_products_id' => $wraplist->fields['orders_products_id'],
                                  'wrap' => $wraplist->fields['wrap'],
                                  'wrap_config' => $wraplist->fields['wrap_config'], 
                                  'wrap_name' => $wraplist->fields['wrap_name']);
          $wraplist->MoveNext();
        }
      }
      // END FEC
    ...
    function cart() {
      ...
      $customer_address_query = "select c.customers_firstname, c.customers_lastname, c.customers_telephone,
                                      ...
                                      co.address_format_id, ab.entry_state, ab.entry_telephone
      ...
      $shipping_address_query = "select ab.entry_firstname, ab.entry_lastname, ab.entry_company,
                                      ...
                                      c.countries_iso_code_3, c.address_format_id, ab.entry_state, ab.entry_telephone
  ...
  */
      ...
      $this->delivery = array('firstname' => $shipping_address->fields['entry_firstname'],
                              ...
                              'shipping_telephone' => $shipping_address->fields['entry_telephone']);
      ...
      $this->billing = array('firstname' => $billing_address->fields['entry_firstname'],
                             ...
                             'billing_telephone' => $billing_address->fields['entry_telephone']);
      ...
      for ($i=0, $n=sizeof($products); $i<$n; $i++) {
        ...
        if (STORE_PRODUCT_TAX_BASIS == 'Shipping' && stristr($_SESSION['shipping']['id'], 'storepickup') == TRUE) {
          ...
          $taxDescription = zen_get_tax_description($products[$i]['tax_class_id'], STORE_COUNTRY, STORE_ZONE);
          $taxRate = zen_get_tax_rate($products[$i]['tax_class_id'], STORE_COUNTRY, STORE_ZONE);
        } else {
          ...
          $taxDescription = zen_get_tax_description($products[$i]['tax_class_id'], $tax_address->fields['entry_country_id'], $tax_address->fields['entry_zone_id']);
          $taxRate = zen_get_tax_rate($products[$i]['tax_class_id'], $tax_address->fields['entry_country_id'], $tax_address->fields['entry_zone_id']);
        }
        
        $this->products[$index]['tax_groups'] = $taxRates;
        $this->products[$index]['tax_description'] = $taxDescription;
  ...
  */
    ...
    function create($zf_ot_modules, $zf_mode = 2) {
      ...
      $sql_data_array = array('customers_id' => $_SESSION['customer_id'],
                              ...
                              'ip_address' => $_SESSION['customers_ip_address'] . ' - ' . $_SERVER['REMOTE_ADDR'],
                              'shipping_telephone' => $this->delivery['shipping_telephone']
                              );
                              
      if ($_SESSION['COWOA']) $sql_data_array[COWOA_order] = 1; // FEC
      ...
      for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
        ...
        if (STOCK_LIMITED == 'true') {
          ...
          if ($stock_values->RecordCount() > 0) {
            ...
            } elseif (defined('TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK') && $attribute_stock_left <= STOCK_REORDER_LEVEL) {
        ...
        if (FEC_GIFT_WRAPPING_SWITCH == 'true') {
          ...
          for ($q = 1; $q <= $this->products[$i]['qty']; $q++) {
              if ( !isset($wrapsettings[$prid][$q]['setting']) || 
                 ($wrapsettings[$prid][$q]['setting'] == 0) ) { 
                 continue;
              }
              $paper = $wrapsettings[$prid][$q]['paper'];
              if ($_SESSION['wrapconfig'] == 'Images') {
                 $wrap_config = 'I'; // Images
                 $wrap_name = $paper; 
              } else if ($_SESSION['wrapconfig'] == 'Descriptions') {
                 $wrap_config = 'D'; // Descriptions
                 $wrap_name = $paper; 
              } else {
                 $wrap_config = 'C'; // Checkbox 
                 $wrap_name = '';
              }
              $sql_data_array = array('orders_id' => $zf_insert_id,
                                      'orders_products_id' => $order_products_id,
                                      'wrap' => $wrapsettings[$prid][$q]['setting'], 
  ...
  /* END: ADD MY CUSTOM DETAILS */
        ...
        // bof giftwrap 
        if ($wrap_count == 0) {
          $products_ordered_wrapped = ''; 
          $html_products_ordered_wrapped = '';
        } else {
          $products_ordered_wrapped = "\t". EMAIL_TEXT_GIFTWRAP . $wrap_count; 
          $html_products_ordered_wrapped = '<br /><small><em> '. EMAIL_TEXT_GIFTWRAP . $wrap_count . '</em></small>';
        }
        // eof giftwrap
        ...
        $this->products_ordered .= $products_ordered_wrapped . "\n";    // giftwrap
    ...
    function send_order_email($zf_insert_id, $zf_mode) {
      ...
      $html_msg['INTRO_DATE_ORDERED']    = strftime(DATE_FORMAT_SHORT);
      ...
      //  $html_msg['EMAIL_TEXT_HEADER'] = EMAIL_TEXT_HEADER;
      $html_msg['EXTRA_INFO'] = '';
      $email_order = html_entity_decode($email_order);
