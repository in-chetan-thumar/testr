  ...
  while (!$listing->EOF) {
    ...
    for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
      ...
      switch ($column_list[$col]) {
        ...
        case 'PRODUCT_LIST_NAME':
        $lc_align = '';
		/*$products_description = ltrim(substr(zen_trunc_string(zen_clean_html(stripslashes(zen_get_products_description($listing->fields['products_id'], $_SESSION[
		'languages_id']))), PRODUCT_LIST_DESCRIPTION), 0, 39) . '...'); //Trims and Limits the desc */
		
		$products_description = ltrim(substr(zen_trunc_string(zen_clean_html(stripslashes(zen_get_product_small_desc($listing->fields['products_id'], $_SESSION[
		'languages_id']))), PRODUCT_LIST_DESCRIPTION), 0, 39) . '...'); //Trims and Limits the desc
		
		$product_name = ltrim(substr($listing->fields['products_name'], 0, 39) . '...'); //Trims and Limits the Product Name
		...
		and 	
		$_GET['filter_id'] > 0) ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : 		zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '">' . $product_name . '</a></div><div class="product_desc"><p class="s_desc">' . $products_description . '</p></div>';
		//.'<div class="productbtn"><a class="product-details" href="'. zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id'] > 0) ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '">Product Details</a></div>';
        break;
        case 'PRODUCT_LIST_MANUFACTURER':
        $lc_align = '';
        ...
        break;
        case 'PRODUCT_LIST_PRICE':
        $lc_price = '<div class="propricemain"><div class="prodprice">' . zen_get_products_display_price($listing->fields['products_id']) . '<br /></div>';
        $lc_align = 'right';
        $lc_text =  $lc_price .'<div class="productbtn"><div class="mj-productdetailimage"><a href="'. zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id'] > 0) ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '">More</a></div></div><div style="float:left; margin-left: 5px; ">
			<a href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $listing->fields['products_id']) . '">'.
					zen_image($template->get_template_dir("buynow.gif", DIR_WS_TEMPLATE, $current_page_base,'images'). '/' . "buynow.gif", "Buy now").
			'</a>
		</div>';;
