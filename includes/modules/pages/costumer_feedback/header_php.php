...
if (isset($_GET['action']) && ($_GET['action'] == 'send')) {
  ...
  if ($zc_validate_email and !empty($fname) and !empty($lname) and !empty($country) and !empty($telephone)) {
    ...
    // Send message
    zen_mail($send_to_name, $send_to_email, "Customer Feedback", $text_message, $name, $email_address, $html_msg,'costumer_feedback');

    zen_redirect(zen_href_link(FILENAME_CUSTOMER_FEEDBACK, 'action=success'));
  } else {
    $error = true;
    /*if (empty($fname)) {
...
$define_page = zen_get_file_directory(DIR_WS_LANGUAGES . $_SESSION['language'] . '/html_includes/', FILENAME_DEFINE_CUSTOMER_FEEDBACK, 'false');
