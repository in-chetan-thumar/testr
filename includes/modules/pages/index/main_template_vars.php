...
{
  $sql = "SELECT cd.categories_name, c.categories_small_desc, c.categories_image
  ...
  if (isset($cPath) && strpos($cPath, '_'))
  {
    ...
    for($i=0, $n=sizeof($category_links); $i<$n; $i++)
    {
      ...
      if ($categories->fields['total'] < 1)
      {
        ...
        $categories_query = "SELECT c.categories_id, cd.categories_name, c.categories_small_desc, c.categories_image, c.parent_id
                             FROM   " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd
                             WHERE      c.parent_id = :parentID
                             AND        c.categories_id = cd.categories_id
                             AND        cd.language_id = :languagesID
