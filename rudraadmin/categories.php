    ...
    switch ($action) {
      ...
      if ($action == 'insert_category') {
        $insert_sql_data = array('parent_id' => (int)$current_category_id,
								'categories_small_desc' => $_POST['categories_small_desc'],
                                 'date_added' => 'now()');
        ...
        $update_sql_data = array('last_modified' => 'now()', 'categories_small_desc' => $_POST['categories_small_desc']);
...
<?php
  ...
  switch ($action) {
    ...
    for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
      $category_inputs_string .= '<br />' . zen_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;';
      $category_inputs_string .= zen_draw_textarea_field('categories_small_desc', 'soft', '100%', '20', htmlspecialchars($cInfo->categories_small_desc, ENT_COMPAT, CHARSET, TRUE));
    }
    $contents[] = array('text' => '<br /> Categories small description:' . $category_inputs_string);
