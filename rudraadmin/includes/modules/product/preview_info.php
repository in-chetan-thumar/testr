  ...
  die('Illegal Access');
    ...
    if (zen_not_null($_POST)) {
      ...
      $product = $db->Execute("select p.products_id, pd.language_id, pd.products_name,
                                      pd.products_description, pd.products_description1, pd.product_small_desc, pd.products_url, p.products_quantity,
    ...
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
      if (isset($_GET['read']) && ($_GET['read'] == 'only')) {
		...
		$pInfo->product_small_desc = zen_get_product_small_desc($pInfo->products_id, $languages[$i]['id']);
		...
		$pInfo->product_small_desc = zen_db_prepare_input($pInfo->products_id, $languages[$i]['id']);
...
/* Re-Post all POST'ed variables */
      ...
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
		...
		echo zen_draw_hidden_field('product_small_desc[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($product_small_desc[$languages[$i]['id']])));
